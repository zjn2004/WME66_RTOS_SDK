...
iks_delete_all (iks *x)
{
    ...
    {
        child = iks_child(x); 
        while(child && IKS_TAG == child->type){
    	    tchild = child;
    	    child = iks_next_tag(child);
    	    iks_delete_all (tchild);
        }
        iks_stack_delete (x->s);
    }
}
...
iks * ICACHE_FLASH_ATTR
iks_find_child(iks *x, const char *name)
{
    if (x == NULL || name == NULL)
        return NULL;
    for (x = iks_child(x); x; x = iks_next_tag(x))
        if (!iks_strcmp(iks_name(x), name))
            break;
    return x; 
}
