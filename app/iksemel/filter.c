	...
	for (i = 0; i < 6; i++)	{
		...
		switch (iter_type) {
			...
			case IKS_RULE_SUBTYPE:
				...
				break;
			case IKS_RULE_ID:
				rule->id = iks_stack_strdup (s, id, 0);
				break;
			case IKS_RULE_NS:
				rule->ns = iks_stack_strdup (s, ns, 0);
				break;
			case IKS_RULE_FROM:
				rule->from = iks_stack_strdup (s, from, 0);
				break;
			case IKS_RULE_FROM_PARTIAL:
				rule->from = iks_stack_strdup (s, from_partial, 0);
				break;
		}
	}
